(function() { // Обернем все в анонимную функцию, чтобы этот код не был выполнен, а был только примером
    ...
    rm.onload = () => { // Передача асинхронная, нужен обработчик принятия
        ...
        /*
        Список идентификаторов в порядке возрастания идентификатора текстуры
        Оптимален для отрисовки, т.к. идущие подряд спрайты имеют одинаковые текстуру и дескриптор
         */
        const idList = gm.optimalIdList(type);
        const id = idList[0]; // Идентификатор объекта
